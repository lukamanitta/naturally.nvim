...
local wdays = { 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday' }
...
local function parse_tz(s, d)
    ...
    if not offset then
        error('expected time zone, got "' .. s .. '"')
    end
...
function node:T(fmt)
    return tostring(self):T(fmt)
end
function node:D(fmt)
    return tostring(self):D(fmt)
end
function node:TS(fmt)
    return tostring(self):TS(fmt)
end
...
local dateparse_parse = {
    ...
    Returns = {
        { Desc = 'Time as a Unix Epoch time value <u>unix epoch time</u>' },
...
local node_D = {
    ...
    Returns = {
        { Desc = 'Time as database date without time <u>string</u>' },
        { Desc = [[Time as a table containing date/time components <u>table</u>]] },
    },
    Examples = {
        [[-- using the dateparse :D() node function
